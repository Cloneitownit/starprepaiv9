// api/proxy-audio.js — v3 Audio proxy with Replicate auth + ElevenLabs support
export default async function handler(req, res) {
  ...
  try {
    ...
    // Add auth token for Replicate API URLs (they require authentication)
    if (hostname === 'api.replicate.com' && process.env.REPLICATE_API_TOKEN) {
      fetchHeaders['Authorization'] = 'Bearer ' + process.env.REPLICATE_API_TOKEN;
    }

    // Add auth token for ElevenLabs API URLs
    if ((hostname === 'api.elevenlabs.io' || hostname.endsWith('.elevenlabs.io')) && process.env.ELEVENLABS_API_KEY) {
      fetchHeaders['xi-api-key'] = process.env.ELEVENLABS_API_KEY;
    }
